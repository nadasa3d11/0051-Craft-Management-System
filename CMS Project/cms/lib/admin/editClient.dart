...
class _EditClientScreenState extends State<EditClientScreen> {
  ...
  void initState() {
    super.initState();
    fullNameController = TextEditingController(text: widget.client["Full_Name"] ?? '');
    ...
    try {
      birthDateController = TextEditingController(
        text: widget.client["Birth_Date"] != null
            ? DateFormat('dd/MM/yyyy').format(DateTime.parse(widget.client["Birth_Date"]))
            : '',
      );
    } catch (e) {
      birthDateController = TextEditingController(text: '');
    }
    gender = widget.client["Gender"] ?? "Male";
  }
  ...
  Future<void> updateClient() async {
    bool hasChanges = fullNameController.text != (widget.client["Full_Name"] ?? '') ||
        phoneController.text != (widget.client["Phone"] ?? '') ||
        addressController.text != (widget.client["Address"] ?? '') ||
        birthDateController.text !=
            (widget.client["Birth_Date"] != null
                ? DateFormat('dd/MM/yyyy').format(DateTime.parse(widget.client["Birth_Date"]))
                : '') ||
        gender != (widget.client["Gender"] ?? "Male");
      ...
      if (result.containsKey("error")) {
        ...
        showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            content: Text(result["message"] ?? "Client updated successfully"),
            actions: [
              TextButton(
                onPressed: () {
  ...
  Future<void> selectDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
      context: context,
      initialDate: widget.client["Birth_Date"] != null
          ? DateTime.parse(widget.client["Birth_Date"])
          : DateTime.now(),
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: isLoading
            ...
            : RefreshIndicator(
                onRefresh: () async {
                  fullNameController.text = widget.client["Full_Name"] ?? '';
                  phoneController.text = widget.client["Phone"] ?? '';
                  addressController.text = widget.client["Address"] ?? '';
                  birthDateController.text = widget.client["Birth_Date"] != null
                      ? DateFormat('dd/MM/yyyy').format(DateTime.parse(widget.client["Birth_Date"]))
                      : '';
                  setState(() {
                    gender = widget.client["Gender"] ?? "Male";
